...
Come segnale in ingresso si è scelto di generare un'onda sinusoidale di ampiezza $500 mV$ e frequenza di $1kHz$. In uscita ci si aspettava un alternarsi di saturazioni dell'amplificatore, a segno alterno ($ \approx 15 V$ per sinusoide negativa, $\approx -15 V$ per sinusoide positiva).
...
\caption{Nell'immagine si apprezza la concordanza di segno tra segnale in ingresso e segnale in uscita. In rosso è riportato il livello di zero crossing della configurazione invertente per dimostrare qualitativamente che il livello di l'amplificazione rimane costante}
\label{fig:open_loop_non_inv_zc}
\end {center}
\end{figure}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Configurazione ad anello aperto con soglia arbitraria} %Stefano
...
La configurazione ad anello aperto dell'amplificatore può essere utile anche per discriminare segnali in ingresso rispetto ad una soglia arbitraria, diversa da $0$.
...
Un segnale sinusoidale di ampiezza attesa $V_{pp} = 5.00 V$ è stato inserito nell'ingresso non invertente. Ci si aspetta dunque un'amplificazione non invertente.
...
Dal confronto tra l'immagine \ref{fig:biased_cross} e l'immagine \ref{fig:open_loop_non_inv_zc} si può notare come in questo caso la soglia non si trovi a livello di $0 V$, e questo risulti in zone di saturazione e
...
\caption{Amplificatore reazionato invertente: guadagno in funzione della frequenza}
...
\subsection{\textbf{Caratterizzazione}}
In laboratorio si è considerata la configurazione citata in \textit{Fig.\ref{fig:loop-non-inv}} con l'obiettivo di ottenere un guadagno pari a due. Sono riportati i dati nella \textit{tabella \ref{tab:opa-non-inv}}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Amplificatore derivatore reale} %Matteo
\subsection{\textbf{Circuito}}
\begin{figure}[H]%[!ht]
\begin {center}
...
In questo caso dato che la curva positiva sarà rappresentata dall'ingresso in sottrazione, mentre quella positiva dall'ingresso non in sottrazione ci si aspetta un valore negativo di coefficiente angolare $a_e = -1$
 \begin{figure}[H]%[!ht]
\begin {center}
\includegraphics[width=0.50\textwidth]{analysis/output/OPA-linfit-inv.pdf}
\caption{Regressione lineare ($y$ = $ax$ +$b$) del canale invertente in modalità $X-Y$ dell'oscilloscopio ($Ch_in-Ch-out$)}
\label{fig:lin-inv}
\end {center}
\end{figure}
\[a = -1.0069 \pm 0.0036 \]
\[b = (0.0101\pm 0.0011) V \]
...
In particolare quando $V_{in}$ è leggermente superiore alla tensione di riferimento, $V_{ref}$, la $V_{out}$ diventa $-V_{sat}$ e nel caso in cui sia inferiore diventerà invece $V_{sat}$. La tensione di input a cui avviene il cambio può essere controllata attraverso i valori di $R_1$ e $R_2$.
...
Con l'utilizzo di un secondo amplificatore operazionale è stato implementato il circuito in \textit{Fig. \ref{fig:schmitt-trigger}}. Un primo OpAmp è collegato in configurazione di mixer. La sua uscita è collegata a uno degli ingressi del secondo OpAmp in configurazione di \textit{trigger di Schmitt}. 

\begin{figure}[H]%[!ht]
\begin{center}
\includegraphics[width=0.45\textwidth]{sch-simulations/output/OPA-mixer-trigger.pdf}
\caption{Schema elettrico utilizzato per la caratterizzazione del trigger di Schmitt con l'obiettivo di mostrare le capacità di immunità al rumore. Il circuito di dotato di due stadi operazionali, il primo con funzione di sommatore produce un segnale rumoroso sommando un noise ad alta frequenza ad un segnale sinusoidale, il secondo costituisce il trigger vero e proprio. Sono stati usati i seguenti valori di resistenza: $R_1 =(9.81 \pm 0.13) \ k\Omega, \ R_2 = (9.90 \pm 0.13) \ k\Omega ,\ R_3 = (9.80 \pm 0.12) \ k\Omega \, \ R_4 = (9.82 \pm 0.13) \ k\Omega \ R = (9.83 \pm 0.13) \ k\Omega , \ R = (9.83 \pm 0.13 ) \ k\Omega $  } 
\label{fig:schmitt-trigger}
\end{center}
\end{figure}
 
Inizialmente è stato fornito al \textit{trigger di Schmitt} un segnale sinusoidale con rumore trascurabile (\textit{"segnale pulito")}. In uscita dal trigger si osserva il comportamento tipico in modalità invertente (\textit{Fig.\ref{fig:trigger-clean}}): 
quando il segnale supera la tensione $V_{LT} $ l'uscita scatta a $ + V_{sat}$ e ci rimane fino a quando il segnale non supera $V_{UT} $ facendo scattare l'uscita alla tensione $- V_{sat}$. Tipicamente un circuito di questo tipo è utilizzato, ad esempio, per trasformare un segnale sinusoidale in un onda quadra.
Nel circuito implementato la tensione di saturazione, riferita a massa è:
\[ +V_{sat}  = 14.6 \pm 0.1  V \]
\[ -V_{sat}  = -14.0 \pm 0.1  V \]
...
Successivamente è stato inviato in ingresso al trigger un segnale sinusoidale sommato - tramite l'amplificatore mixer presente nel circuito - ad un rumore ad alta frequenza (\textit{segnale rumoroso}). E' stato studiato il comportamento del trigger in due configurazioni: anello aperto e anello chiuso.

Nel primo caso \textit{il trigger di Schmitt} si comporta come un qualunque comparatore ad anello aperto: in presenza di un segnale disturbato, in corrispondenza della soglia, non si ha più una commutazione netta dell'uscita, ma una serie di commutazioni dovute ai ripetuti superamenti di soglia del rumore. Ciò è ben visualizzabile in \textit{figura \ref{fig:trigger-noisy}}, dove si ha una commutazione indesiderata in corrispondenza del punto in cui il rumore supera la soglia ($0 \ V)$ in due istanti consecutivi. 
\begin{figure}[H]%[!ht]
\begin {center}
...
\end {center}
\end{figure}
La retroazione positiva introdotta nella configurazione ad anello chiuso, rimedia a questo problema introducendo due soglie diverse, a seconda che la tensione di uscita valga $+V_{sat}$ o $-V_{sat}$. L'effetto che si ottiene è che anche in presenza di rumore la commutazione è "protetta" dalla banda compresa tra la due tensioni di soglia: affinché avvenga uno scatto, la tensione deve andare al di sopra della soglia alta o al di sotto della soglia bassa.
La stessa onda "rumorosa" inviata precedentemente al trigger, questa volta non genera più alcuna commutazione indesiderata (\textit{fig. \ref{fig:trigger-ok}}).
...
Si ricorda che l'isteresi previene il trigger dall'oscillare quando un input che varia a bassa frequenza si trova vicina al \textit{threshold voltage}. Aumentando la frequenza ci si aspetta perciò una minore efficacia del comparatore come visibile qua di seguito (in \textit{figg. \ref{fig:hyst-table} e \ref{fig:trigger-hyst}} vi sono gli stessi dati in due diverse rappresentazioni, affiancati o sovrapposti).
\begin{figure}[H]%[!ht]
\begin {center}
...
\end {center}
\end{figure}
\[\Delta V_t  = V_{LT}-V_{UT} = (14.424 \pm 0.036)  V \]
\[\Delta V_{sat}  = V_{sat_+}-V_{sat_{-}} = (28.283 \pm 0.101(1))  V \]
...
\end {center}
\end{figure}
Dai due grafici si osserva chiaramente che ad alta frequenza la velocità di commutazione del circuito diventa insufficiente. Questo effetto è frutto dello slew rate dell'amplificatore operazionale utilizzato.
%%%%%%%%%%FINE TERZO GIORNO%%%%%%%%%%%%%%%%%%
...
\hline  \\
64.0 $\pm$  4.0 & 114.0 $\pm$ 4.0 & 1.0 $\pm$ 0.001        \\ 
61.6 $\pm$ 4.0  & 116.0 $\pm$ 4.0 & 10.0 $\pm$ 0.01        \\ 
61.1 $\pm$ 4.0  & 121.0 $\pm$ 4.0 & 100.0 $\pm$ 0.1        \\ 
60.8 $\pm$ 4.0  & 119.0 $\pm$ 4.0 & 1000.0 $\pm$ 1.0       \\ 
60.8 $\pm$ 4.0  & 119.0 $\pm$ 4.0 & 5000.0 $\pm$ 5.0       \\ 
56.8 $\pm$ 4.0  & 112.0 $\pm$ 4.0 & 13000.0 $\pm$ 13.0     \\ 
61.6 $\pm$ 4.0  & 119.0 $\pm$ 4.0 & 50000.0 $\pm$ 50.0     \\ 
55.2 $\pm$ 4.0  & 104.0 $\pm$ 4.0 & 100000.0 $\pm$ 100.0   \\ 
57.6 $\pm$ 4.0  & 106.0 $\pm$ 4.0 & 200000.0 $\pm$ 200.0   \\ 
52.8 $\pm$ 4.0  & 90.4 $\pm$ 4.0  & 350000.0 $\pm$ 350.0   \\ 
54.4 $\pm$ 4.0  & 81.6 $\pm$ 4.0  & 500000.0 $\pm$ 500.0   \\ 
56.8 $\pm$ 4.0  & 77.6 $\pm$ 4.0  & 600000.0 $\pm$ 600.0   \\ 
54.4 $\pm$ 4.0  & 67.2 $\pm$ 4.0  & 750000.0 $\pm$ 750.0   \\ 
55.2 $\pm$ 4.0  & 54.4 $\pm$ 4.0  & 1000000.0 $\pm$ 1000.0 \\ 
55.2 $\pm$ 4.0  & 33.6 $\pm$ 4.0  & 1700000.0 $\pm$ 1700.0 \\ 
56.8 $\pm$ 4.0  & 21.6 $\pm$ 4.0  & 2500000.0 $\pm$ 2500.0 \\ 
56.8 $\pm$ 4.0  & 15.2 $\pm$ 4.0  & 3300000.0 $\pm$ 3300.0 \\ 
57.6 $\pm$ 4.0  & 8.8 $\pm$ 4.0   & 5000000.0 $\pm$ 5000.0 \\ 
\end{tabular}
\vspace{5 mm}
...
\multicolumn{1}{c|}{$V_{in}$ {[}V{]}} & \multicolumn{1}{|c|}{$V_{out}$ {[}V{]}} & \multicolumn{1}{c}{$f$ {[}Hz{]}}\\ 
\hline  \\
80.8 $\pm$ 4.0 & 79.2 $\pm$ 4.0 & 100.0 $\pm$ 0.1      \\
81.6 $\pm$ 4.0 & 78.4 $\pm$ 4.0 & 150.0 $\pm$ 0.15     \\
80.8 $\pm$ 4.0 & 77.6 $\pm$ 4.0 & 200.0 $\pm$ 0.2      \\
80.8 $\pm$ 4.0 & 74.4 $\pm$ 4.0 & 300.0 $\pm$ 0.3      \\
80.8 $\pm$ 4.0 & 69.6 $\pm$ 4.0 & 400.0 $\pm$ 0.4      \\
80.8 $\pm$ 4.0 & 68.0 $\pm$ 4.0 & 450.0 $\pm$ 0.45     \\
80.8 $\pm$ 4.0 & 64.8 $\pm$ 4.0 & 500.0 $\pm$ 0.5      \\
80.8 $\pm$ 4.0 & 56.8 $\pm$ 4.0 & 700.0 $\pm$ 0.7      \\
80.8 $\pm$ 4.0 & 47.2 $\pm$ 4.0 & 1000.0 $\pm$ 1.0     \\
80.8 $\pm$ 4.0 & 41.6 $\pm$ 4.0 & 1200.0 $\pm$ 1.2     \\
80.8 $\pm$ 4.0 & 35.2 $\pm$ 4.0 & 1500.0 $\pm$ 1.5     \\
80.0 $\pm$ 4.0 & 28.0 $\pm$ 4.0 & 2000.0 $\pm$ 2.0     \\
80.0 $\pm$ 4.0 & 23.2 $\pm$ 4.0 & 2500.0 $\pm$ 2.5     \\
80.0 $\pm$ 4.0 & 19.2 $\pm$ 4.0 & 3000.0 $\pm$ 3.0     \\
80.0 $\pm$ 4.0 & 16.8 $\pm$ 4.0 & 3500.0 $\pm$ 3.5     \\
80.0 $\pm$ 4.0 & 15.2 $\pm$ 4.0 & 4000.0 $\pm$ 4.0     \\
80.0 $\pm$ 4.0 & 12.0 $\pm$ 4.0 & 5000.0 $\pm$ 5.0     \\
80.0 $\pm$ 4.0 & 11.2 $\pm$ 4.0 & 5500.0 $\pm$ 5.5     \\
80.0 $\pm$ 4.0 & 10.4 $\pm$ 4.0 & 6000.0 $\pm$ 6.0     \\
80.8 $\pm$ 4.0 & 8.8 $\pm$ 4.0  & 7000.0 $\pm$ 7.0     \\
80.8 $\pm$ 4.0 & 7.2 $\pm$ 4.0  & 10000.0 $\pm$ 10.0   \\
80.8 $\pm$ 4.0 & 5.6 $\pm$ 4.0  & 14000.0 $\pm$ 14.0   \\
80.8 $\pm$ 4.0 & 4.0 $\pm$ 4.0  & 20000.0 $\pm$ 20.0   \\
80.0 $\pm$ 4.0 & 2.4 $\pm$ 4.0  & 50000.0 $\pm$ 50.0   \\
80.8 $\pm$ 4.0 & 1.6 $\pm$ 4.0  & 100000.0 $\pm$ 100.0
\end{tabular}
\vspace{5 mm}
