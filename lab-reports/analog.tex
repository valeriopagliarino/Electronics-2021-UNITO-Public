...
\[BAND WIDTH \dot GAIN\]
ottenendo i valori \[H.F_{2}\times Gain_2 = 727612.75410076 Hz\]  \[H.F_{11}\times Gain_{11} = 600850.4939452771 Hz\] (\ref{fig:gain-compare})
\begin{figure}[H]%[!ht]
\begin {center}
...
\end {center}
\end{figure}
Impostando una frequenza, $f=valore_da _controllare$, si è proceduto con l'osservazione della risposta del circuito, \ref{fig:OPA-integ-res}. Come atteso la $V_{out}$ risulta essere molto vicina all'integrale del segnale in ingresso. In particolare si nota l'affinità con l'integrale calcolato numericamente rappresentato dalla linea rossa tratteggiata.
...
\[V_{out} = - R  \sum_{n} \frac{V_n}{R_n} = - \sum_{n} V_n \tag{1} \] L'ultimo passaggio è possibile scegliendo tutti i valori delle resistenze uguali ($R = R_1 = R_2$). Nel nostro caso si ha: \textbf{!inserire valori resistenza!}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Amplificatore di differenze invertente} %Federico
...
\caption{Circuito amplificatore di differenze invertente}
\label{fig:differenze}
\end {center}
\end{figure}

\subsubsection{Differenze di onde sinusoidali}
