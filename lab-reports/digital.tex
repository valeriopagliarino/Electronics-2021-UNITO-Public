...
I seguenti istogrammi rappresentano le frequenze dei codici in 8 prese dati a differenti frequenze di clock. La linea rossa tratteggiata indica il valore medio delle frequenze.
Affinché il dispositivo generi tutti i codici necessari il range di variazione del segnale di input deve essere superiore rispetto a quello di \textit{clipping}. Per questa ragione si è scelto di utilizzare una forma d'onda a rampa con estremi -4.5-0.5 V. Inoltre si è applicato uno \textit{sweep} di frequenza da 5 Hz a 50 Hz eseguito in $\Delta T$ = 35 s
con lo scopo di evitare che la scelta accidentale di frequenze multiple e sottomultiple di quella di campionamento influissero sulla veridicità dei dati presi.
Infine i condici pari allo 0 ed al $2^n$, che nel caso specifico corrisponderà a 16, sono stati esclusi dalla conta a causa del numero non valido di occorrenze.
I dati misurati infine sono stati normalizzati rispetto al valore medio di ricorrenze.
%% continua ...
...
\subsection{Distribuzione della deviazione standard}
Dai grafici soprastanti si nota come la linearità del dispositivo sia mantenuta solamente per frequenze di campionamento limitate. Dalla \textit{figura \ref{fig:graph_dnl_3000_hz}} alla \textit{figura \ref{fig:graph_dnl_8000_hz}} infatti si osservano variazioni rispetto al valor medio superiori al 10\%.
In particolare nella \textit{figura \ref{fig:graph_dnl_sigma}} si sottolinea come la deviazione standard dei valori dei vari codici si attesti su un valore di plateau entro valori di frequenze di Clock minori di 1 Khz per poi aumentare fino a livelli pari al 16 \% del valore medio di occorrenze dei codici. 
\begin{figure}[H]%[!ht]
\begin{center}
...
Per accertarsi di quale fosse la principale causa della perdita di linearità all'aumento della frequenza di campionamento sono stati scambiati i collegamenti dei bit in uscita dall'Arduino verso il Raspberry. 
Confrontando i risultati ottenuti nelle \textit{figure \ref{fig:graph_dnl_8000_hz}} e \textit{\ref{fig:graph_dnl_non_lin}} si vede come i codici con maggiori ricorrenze non siano analoghi nei due casi. Ciò suggerisce un limite della parte digitale che potrebbe essere dovuto ad una R2R non perfettamente calibrata, condizione comprensibile a causa della limitata uguaglianza fra le resistenze impiegate.
\begin{figure}[H]%[!ht]
\begin{center}
