...
%QUESTI VALORI SONO INCONSISTENTI, IMMAGINO SI TRATTI DI UN TESTO
%SEGNAPOSTO
...
\end{center}
\end{figure}
Per un buon funzionamento del circuito è inoltre necessario che la capacità del condensatore deve essere limitata il maniera tale da permettere il rapido \textit{aggancio} del segnale in ingresso nella transizione da sample a hold.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{itemize}
Negli ADC dunque il segnale analogico è \textit{campionato}, cioè prelevato ad intervalli regolari di tempo e convertito in digitale per mezzo della quantizzazione, durante la quale verrà approssimata l'ampiezza del segnale analogico attraverso un numero finito di combinazioni di bit.
\subsection{Verifica del teorema del campionamento di Nyquist-Shannon con sample-hold e oscilloscopio}
Risulta necessario capire quale sia la frequenza alla quale è possibile campionare un segnale senza perdere informazioni. Il \textit{Teorema del campionamento di Shannon} afferma che sia possibile ricostruire un segnale a partire dai suoi campioni se la frequenza di campionamento $f_c$ è almeno doppia rispetto al limite superiore $f_{MAX}$ dello spettro del segnale considerato nel suo  dominio di frequenza, secondo il teorema di Fourier. 
\[f_c \ge 2 f_{MAX}\]
Attraverso questa condizione analitica il teorema di Shannon evince quale sia il limite inferiore della $f_c$ per non perdere informazioni. A frequenze inferiori si presenterà il fenomeno di \textit{aliasing}, per cui sfruttando $f_c$ troppo basse si ottengono dei  campioni che non sono solo più affini al segnale effettivo, corrispondente alla frequenza maggiore dello spettro, ma anche ad onde con periodi maggiori, rendendo la ricostruzione non più univoca. 
Se si conoscesse il tempo di conversione sarebbe possibile ricavare la frequenza massima che deve avere il segnale  per limitare l'errore a un quanto, ma ond'evitare di limitare eccessivamente la banda dell'ADC conviene \textit{bloccare} il segnale durante la conversione con un circuito si sample and hold, descritto nel paragrafo precendente.

Dalla \textit{figura \ref{fig:sampSH1}} alla \textit{figura \ref{fig:sampSH6}} sono riportati i segnali in visione sull'oscilloscopio. E' da notare che il ritardo dell'output,dovuto al tempo necessario allo shift register per scorrere tutti i bit, è stato eliminato mediante traslazione del segnale MTH , derivato dalla moltiplicazione tra il segnale in uscita dal DAC ed il segnale di conversione completata. Ciò è appunto dovuto al tempo necessario allo shift register per scorrere tutti i bit. Si osserva inoltre come la curva generata dai picchi del segnale di output sia un ottima approssimazione del segnale in ingresso, complice il fatto di una $f_c$ molto maggiore rispetto a quella dell'input % aggiungere valori frequenze 
\clearpage
\clearpage
\begin{figure}[H]%[!ht]
\begin{center}
...
\end{document}    \item Grazie all'amplificazione dovuta alla configurazione ad emettitore comune rende più ripido il fronte di commutazione dell'uscita del comparatore, consentendo all'ADC di lavorare a frequenze più elevate.
