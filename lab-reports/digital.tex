...
\begin{itemize}
    \item l'acquisizione di una forma d'onda sinusoidale con range dinamico tra $V_{max}$ = -0.4 V e $V_{min}$ = -3 V e frequenza f = 50 Hz;
    \item l'acquisizione di una forma d'onda triangolare con range dinamico tra $V_{max}$ = -0.4 V e $V_{min}$ = -3. V e frequenza f = 5 Hz;
...
\begin{figure}[H]%[!ht]
\begin{center}
\includegraphics[trim = {0 25 0 0},clip, width=0.50\textwidth]{analysis/output/mcu_square.pdf}
...
%\[f_c \ge 2 f_{MAX}\]
%Attraverso questa condizione analitica il teorema di Shannon evince quale sia il limite inferiore della $f_c$ per non perdere informazioni. A frequenze inferiori si presenterà il fenomeno di \textit{aliasing}, per cui sfruttando $f_c$ troppo basse si ottengono dei  campioni che non sono più affini al solo segnale effettivo, corrispondente alla frequenza maggiore dello spettro, ma anche ad onde con periodi maggiori, rendendo la ricostruzione non più univoca. 
