...
\end{center}
\end{figure}
Il \textit{Flip-Flop SR }, noto  anche come \textit{SR Latch}, può essere considerato come uno dei circuiti sequenziali più basilari possibili. Sostanzialmente si tratta di un dispositivo di memoria a un bit bistabile. E' dotato di due ingressi quali \textit{SET} e \textit{RESET} rispettivamente significanti uscita al valore logico 1 e 0. L'ingresso di RESET  inoltre ripristina il  Flip-Flop nel suo stato originale. Un circuito Flip-Flop basato sui gate NAND fornisce un feedback da entrambe le sue uscite agli ingressi opposti ed è comunemente sfruttato per la memorizzazione di un singolo Bit di dati. Quindi si può affermare che questo dispositivo presenti tre ingressi, quali SET, RESET e Q, logica in uscita , relativa allo stato corrente ed alla cronologia. Infine il termine Flip-Flop si riferisce al funzionamento effetivo del dispositivo, in quanto quest'ultimo può essere \textit{'capovolto'} in uno stato SET logico o  \textit{'floppato'} nello stato di RESET logico opposto.  
Il comportamento di un SR NAND Flip-Flop è regolato dalla seguente tavola di verità:
\begin{center}
...
\begin{figure}[H]%[!ht]
\begin{center}
\includegraphics[ width=0.37\textwidth]{analysis/output/set-reset-to-0_0.pdf}
\label{fig:SR-00}
\end{center}
\end{figure}
...
\begin{figure}[H]%[!ht]
\begin{center}
\includegraphics[ width=0.37\textwidth]{analysis/output/set-reset-to-1_0.pdf}
\label{fig:SR-10}
\end{center}
\end{figure}
...
\begin{figure}[H]%[!ht]
\begin{center}
\includegraphics[ width=0.37\textwidth]{analysis/output/set-reset-to-1_1.pdf}
\label{fig:SR-11}
\end{center}
\end{figure}
...
\begin{figure}[H]%[!ht]
\begin{center}
\includegraphics[ width=0.37\textwidth]{analysis/output/set-reset-to-0_1.pdf}
\label{fig:SR-01}
\end{center}
\end{figure}
...
Le \textit{figure \ref{fig:SR-1}, \ref{fig:SR-2}} sottolineano il fatto che il dispositivo agisca nella maniera attesa, verificando in ogni situazione i livelli logici predetti.
Questo dispositivo presenta però due grosse limitazioni:
\begin{itemize}
    \item Lo stato S=0 e R=0 deve essere evitato, poichè produce un risultato impredicibile
    \item Se gli stati di S o R cambiano mentre l'input è alto la corretta operazione di latching non avviene
\end{itemize}
...
Il \textit{JK Flip-Flop} è uno dei  Flip-Flop più usati all'interno dei circuiti digitali. Presenta due input \textit{J} e \textit{K} analoghi al SET e RESET del caso  precedente. Tale dispositivo è in grado di superare entrambi i principali problemi del SR Flip-Flop. Infatti si tratta sostanzialmente di un SR Flip-Flop dotato di NAND a tre ingressi, uno dei  quali è occupato dal Clock. Grazie all'aggiunta del Clock vi sono quattro possibili combinazioni, riportate nella tabella di verità sottostante 
